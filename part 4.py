...
#creating the .txt file (part 4)
txt_file=open('OutputList.txt','w')
...
def progression():
  ...
  # Progression outcome
    if(crdt_pass==120):
        print("Progress")
        progress+=1
        txt_file.write("Progress - ")  #write to the text file
        
    elif(crdt_pass==100):
        print("Progress (module trailer)")
        trailer+=1
        txt_file.write("Progress (module trailer) - ")          # write to the text file
        
    elif((crdt_pass==80) or (crdt_pass==60)):
        print("Do not Progress – module retriever")
        retriever+=1
        txt_file.write("Module retriever - ")           #write to the text file
        
    elif((crdt_pass<=40) and (crdt_fail>=80)):
        print("Exclude")
        excluded+=1
        txt_file.write('Exclude - ')        #write to the text file
        
    else:
        print("Do not progress – module retriever")
        retriever+=1
        txt_file.write('Retriever - ')     #write to the text file
...
#--------------Start of the Staff version------------------------------
    ...
    while data_loop:
        ...
        txt_file.write((str(crdt_pass)+', '))
        txt_file.write((str(crdt_defer)+', '))
        txt_file.write((str(crdt_fail)))
        txt_file.write('\n')
   ...
   #--------------- part 2 ---------------------
    ...
    #to print data
    print("-"*60)
    txt_file=open("OutputList.txt",'r')
    print(txt_file.read())
    txt_file.close()
